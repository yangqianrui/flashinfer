...
template <uint32_t HEAD_DIM, PosEncodingMode POS_ENCODING_MODE, typename AttentionVariant,
          typename Params>
cudaError_t BatchDecodeWithPagedKVCacheLocalRemoteDispatched(
...
void BatchDecodeWithPagedKVCacheLocalRemoteRun(
    TensorView float_workspace_buffer, TensorView int_workspace_buffer, Array<int64_t> plan_info_vec,
  ...
  if (kv_layout == QKVLayout::kHND) {
    num_kv_heads = paged_k_cache_local.size(1);
    page_size = paged_k_cache_local.size(2);
  } else {
    page_size = paged_k_cache_local.size(1);
    num_kv_heads = paged_k_cache_local.size(2);
  }
  uint32_t head_dim_qk = q.size(2);
  uint32_t head_dim_vo = paged_v_cache_local.size(3);
  ...
  const int64_t* kv_cache_strides = nullptr;
  auto k_local_strides = paged_k_cache_local.strides();
  auto v_local_strides = paged_v_cache_local.strides();
  auto k_remote_strides = paged_k_cache_remote.strides();
  auto v_remote_strides = paged_v_cache_remote.strides();
  TVM_FFI_ICHECK_EQ(k_local_strides.size(), v_local_strides.size());
  TVM_FFI_ICHECK_EQ(k_local_strides.size(), k_remote_strides.size());
  TVM_FFI_ICHECK_EQ(k_local_strides.size(), v_remote_strides.size());
  for (int i = 0; i < k_local_strides.size(); ++i) {
    TVM_FFI_ICHECK_EQ(k_local_strides[i], v_local_strides[i]);
    TVM_FFI_ICHECK_EQ(k_local_strides[i], k_remote_strides[i]);
    TVM_FFI_ICHECK_EQ(k_local_strides[i], v_remote_strides[i]);
  }
  kv_cache_strides = k_local_strides.data();
  ...
  DISPATCH_context(
      DTypeQ, DTypeKV, DTypeO, IdType, HEAD_DIM_QK, HEAD_DIM_VO, POS_ENCODING_MODE,
      USE_SLIDING_WINDOW, USE_LOGITS_SOFT_CAP, AttentionVariant, ParamsLocalRemote, [&] {
        paged_kv_local_remote_t<DTypeKV, IdType> paged_kv(
            num_kv_heads, page_size, HEAD_DIM_QK, batch_size, kv_layout,
            static_cast<DTypeKV*>(paged_k_cache_local.data_ptr()),
            static_cast<DTypeKV*>(paged_v_cache_local.data_ptr()),
            static_cast<DTypeKV*>(paged_k_cache_remote.data_ptr()),
            static_cast<DTypeKV*>(paged_v_cache_remote.data_ptr()), kv_cache_strides,
            static_cast<IdType*>(paged_kv_indices.data_ptr()),
            static_cast<IdType*>(paged_kv_indptr.data_ptr()),
            static_cast<IdType*>(paged_kv_last_page_len.data_ptr()),
        ...
        cudaError_t status =
            flashinfer::BatchDecodeWithPagedKVCacheLocalRemoteDispatched<
                HEAD_DIM_QK, POS_ENCODING_MODE, AttentionVariant>(params, tmp_v, tmp_s, enable_pdl,
                                                                  /*stream=*/stream);
        TVM_FFI_ICHECK(status == cudaSuccess)
            << "BatchDecodeWithPagedKVCacheLocalRemote failed with error "
